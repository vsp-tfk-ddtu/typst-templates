...
#let signature_field(length: 95%) = {
  grid(
    columns: 1,
    row-gutter: 0.2em,
    [#line(length: length, stroke: 0.4pt)],
    [#align(center)[#text(size: 10pt)[(прізвище та ініціали)]]]
  )
}
  ...
  questions: ()
    ...
    align(center)[
      ...
      #block(above: 1em, below: 1em)[
          #par[#text(weight: "bold")[
            на виконання програми виробничої практики
            в умовах #company_name
          ]]
      ]
    ]
...
#let task_signature() = {
  align(right + bottom)[
    #grid(
      ...
      column-gutter: 1em,
      align: bottom,
      [#par(leading: 0.5em)[Здобувач освіти:]],
      [],[],
      signature_field(length: 100%),
      [#par(leading: 0.5em)[Керівник практики від пидприємства:]],
      [],[],
      signature_field(length: 100%),
      [#par(leading: 0.5em)[Керівник практики від коледжу:]],
      [],[],
      signature_field(length: 100%),
    )
  ]
